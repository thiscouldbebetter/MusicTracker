...
class Song {
    ...
    instrumentAndSequenceLookupsBuild() {
        this.instrumentsByName =
            ArrayHelper.addLookups(this.instruments, (i) => i.name);
    ...
    static fromModFile(modFile) {
        ...
        var song = new Song(this.name, ModFile.SamplesPerSecond, // samplesPerSecond,
    ...
    static fromModFile_Sequences(modFile, instruments) {
        ...
        for (var s = 0; s < sequencesFromModFile.length; s++) {
            ...
            for (var t = 0; t < divisionCellsForChannels.length; t++) {
                ...
                for (var c = 0; c < divisionCellsForChannel.length; c++) {
                    ...
                    if (instrumentIndex != 0) {
                        ...
                        var note = new Note(c, // timeStartInTicks
            ...
            var sequence = new Sequence("_" + s, ticksPerSecond, 64, // durationInTicks
            tracksConverted);
    ...
    uiUpdate_Create() {
        ...
        buttonExport.onclick = () => {
            ...
            var songAsWavFile = Tracker.samplesToWavFile(songFilePath, song.samplesPerSecond, song.bitsPerSample, songAsSamples);
        ...
        buttonInstrumentNew.onclick = () => {
            var now = new Date();
            var nowAsString = StringHelper.padLeft("" + now.getHours(), 2, "0")
        ...
        inputFileToLoad.onchange = (event) => {
            var file = event.target.files[0];
            if (file != null) {
                FileHelper.loadFileAsText(file, (file, instrumentAsJSON) => // callback
                 {
