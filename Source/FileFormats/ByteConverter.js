...
(function (ThisCouldBeBetter) {
    ...
    (function (MusicTracker) {
        class ByteConverter {
            ...
            bytesToIntegerSignedBE(bytes) {
                ...
                for (var i = 0; i < numberOfBytes; i++) {
                    ...
                    returnValue |= byte << (i * MusicTracker.Constants.BitsPerByte);
                }
            ...
            bytesToIntegerSignedLE(bytes) {
                ...
                var numberOfBytes = bytes.length;
                for (var i = 0; i < numberOfBytes; i++) {
                    returnValue |= bytes[i] << (i * MusicTracker.Constants.BitsPerByte);
                }
                if (returnValue > this.maxValueSigned) {
                    returnValue -= this.maxValueUnsigned;
                }
            ...
            integerToBytesBE(integer) {
                ...
                for (var i = 0; i < this.numberOfBytes; i++) {
                    var byteValue = (integer >> (MusicTracker.Constants.BitsPerByte * i)) & 0xFF;
        ...
        MusicTracker.ByteConverter = ByteConverter;
