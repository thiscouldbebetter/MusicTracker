...
(function (ThisCouldBeBetter) {
    ...
    (function (MusicTracker) {
        class FrequencyAnalysis {
            ...
            static fromSamples(numberOfOscillators, frequencyLowestInCyclesPerSecond, samplesPerSecond, samples) {
                var sineAmplitudes = [];
                ...
                for (var i = 0; i < numberOfOscillators; i++) {
                    var oscillatorFrequencyInCyclesPerSecond = frequencyLowestInCyclesPerSecond * i;
                    ...
                    for (var s = 0; s < numberOfSamples; s++) {
                        ...
                        var timeInCycles = timeInSeconds * oscillatorFrequencyInCyclesPerSecond;
            ...
            toSamples(samplesPerSecond, frequencyFundamentalInCyclesPerSecond, durationInSeconds) {
                var samples = [];
                ...
                for (var i = 0; i < this.sineAmplitudes.length; i++) {
                    ...
                    if (sineAmplitude != 0 || cosineAmplitude != 0) {
                        var oscillatorFrequencyInCyclesPerSecond = frequencyFundamentalInCyclesPerSecond * i;
                        for (var s = 0; s < durationInSamples; s++) {
                            var timeInSeconds = s / samplesPerSecond;
                            var timeInCycles = timeInSeconds * oscillatorFrequencyInCyclesPerSecond;
                            var timeInRadians = timeInCycles * radiansPerCycle;
                            var sine = Math.sin(timeInRadians);
                            var cosine = Math.cos(timeInRadians);
                            var sineTimesSineAmplitude = sine * sineAmplitude;
                            var cosineTimesCosineAmplitude = cosine * cosineAmplitude;
        ...
        MusicTracker.FrequencyAnalysis = FrequencyAnalysis;
